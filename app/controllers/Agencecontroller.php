...
class Agencecontroller extends Controller
{
    ...
    {
        $this->particulierModel = $this->model('Particuliersmodels');
    }
    ...
    public function add()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($data['lnameError']) && empty($data['fnameError']) && empty($data['phoneError']) && empty($data['emailError'])) {
                ...
                if ($this->particulierModel->add($data)) {
                    //Redirect to the login page
                    header('location: ' . URLROOT . '/dashboard/particuliers');
                } else {
    ...
    public function edit()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($data['pdp']['error'] == 4) {
                ...
                if (empty($data['lnameError']) && empty($data['fnameError']) && empty($data['phoneError']) && empty($data['emailError'])) {
                    //Register user from model function
                    if ($this->particulierModel->edit($data)) {
                        //Redirect to the login page
                        header('location: ' . URLROOT . '/dashboard/particuliers');
                    } else {
                        die('Something went wrong.');
        ...
        $this->view('/dashboard/particuliers', $data);
    }
    ...
    {
        $data = ['id' => $_POST['id']];
        if ($this->particulierModel->delete($data)) {
            //Redirect to the login page
            header('location: ' . URLROOT . '/dashboard/particuliers');
        } else {
            die('Something went wrong.');
